...
public class DataTypeUtils {
    ...
    public static boolean isExtendPrimitive(Method method) {
        ...
        // 判断是否为基础数据类型
        return isExtendPrimitive(outClass);
    }
    ...
    public static boolean isExtendPrimitive(Class<?> clazz) {
        return isPrimitive(clazz)
            || isPrimitive(typeUnBoxing(clazz))
    ...
    public static Class<?> getaOutParamClass(Method method) {
        ...
        // 请求结果不为空时处理如下逻辑
        Class<?> outClass = null;
        // 判断是否是参数化类型[例如: List<String>\List<Map<String, Object>>\List<User>]
        if (returnType instanceof ParameterizedType) {
            // 参数话类型如: Result<String>\Result<List<String>>\Result<List<List<String>>> 最多需要判断三层来确定参数化类型的具体元素类型
            ParameterizedType parameterizedType = (ParameterizedType) returnType;
            log.trace("返回值类型是参数化类型, 其类型为: {}", parameterizedType);
            // 获取第二层参数
            Type actualTypeArgument = parameterizedType.getActualTypeArguments()[0];
            if (actualTypeArgument instanceof ParameterizedType) {
                outClass = (Class<?>) ((ParameterizedType) actualTypeArgument).getRawType();
                // 如果参数类型还是集合, 则需要再进一步判断集合里的元素类型
                if (Collection.class.isAssignableFrom(outClass)) {
                    // 获取第三层参数
                    Type subActualTypeArgument = ((ParameterizedType) actualTypeArgument).getActualTypeArguments()[0];
                    if (subActualTypeArgument instanceof ParameterizedType) {
                        outClass = (Class<?>) ((ParameterizedType) subActualTypeArgument).getRawType();
                    } else {
                        outClass = (Class<?>) subActualTypeArgument;
                    }
                }
            } else if (actualTypeArgument instanceof WildcardType) {
                // 如果是 Result<?>, 则设置为 Object.class, 由 Jackson 序列化返回
                outClass = Object.class;
            } else if (actualTypeArgument instanceof TypeVariable) {
                // 如果是还是泛型, 则返回原始类型, 由 Jackson 序列化返回
                outClass = method.getReturnType();
            } else {
                // 强转 获取实际参数类型数组, 比如List<Dog>, 则获取的是数组[Dog], Map<Dog, Cat> 则获取的是数组[Dog, Cat]
                outClass = (Class<?>) actualTypeArgument;
            }
        } else {
            log.trace("返回值类型不是参数化类型其类型为: {}", returnType);
            // 不是参数化类型,直接获取返回值类型: 即当返回值是: int/Integer等或实体时
            outClass = method.getReturnType();
        }
    ...
    private static final class CatchDataTypeException {
        ...
        private static <T> void notDecimal(Class<T> clazz, @NotNull BigDecimal targetData) {
            ...
            if (targetData.compareTo(bigDecimal) != 0) {
                throw new DataTypeException(
                    StrFormatter.format("类型为 {} 的数据 {} 为小数,无法转换为类型 {}",
                        targetData.getClass(),
                        targetData,
                        clazz.getName()));
            }
        ...
        private static <T> void isBetweenSpan(Class<T> destClazz, Object srcObj) {
            ...
            if (maxOverflow || minOverflow) {
                throw new DataTypeException(
                    StrFormatter.format("类型为 {} 的数值 {} 不在数据类型 {} 可以容纳的数值范围内",
        ...
        private static <T> DataTypeException returnCouldNotConvertException(Class<T> destClazz, Object srcObj) {
            return new DataTypeException(
                StrFormatter.format("类型为 {} 的数据 {} 无法转换为类型 {}",
